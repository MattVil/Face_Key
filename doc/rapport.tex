...
De nos jours on observe une augmentation du nombre de sites qui nécessite la création d'un compte : réseaux sociaux, différents comptes mail, opérateur téléphonique, journaux en ligne ... Il devient difficile d'avoir un mot de passe différent pour chaque site, qu'il soit assez complexe pour être sécurisé et qu'on n'ait pas besoin de le noter quelque part. La plupart des utilisateurs utilisent donc un seul mot de passe, souvent très simple, en dépit des problèmes de sécurité que cela pose. Leurs comptes et les informations qu'ils contiennent deviennent donc facilement hackable et comme les comptes sont souvent liés, on risque un hacking en chaine de beaucoup de ses comptes.
Une solution développée pour pallier ce problème est celle du gestionnaire de mot de passe (GMDP). Le principe est simple : l'utilisateur n'a qu'un seul mot de passe maître (plus élaboré) à connaitre et c'est le GMDP qui s'occupe des combinaisons identifiant/mot de passe sécurisées pour chaque site. L'un des problèmes de cette solution est qu'il faut toujours retenir un mot de passe maître élaboré pour déverrouiller le GMDP ; ce déverrouillage pouvant prendre plus de temps. Avec cette méthode il peut aussi être fastidieux, contre intuitif et peu sécurisé de partager l'accès à un compte (partager son compte d'opérateur téléphonique avec son conjoint par exemple).
\subsubsection{Maturation de la technologie}
Avec l'émergence du deeplearning et d'autres techniques de traitement d'image et du machine learning, les techniques de reconnaissance faciale deviennent de plus en plus performantes. Certain algorithme arrive même à battre l'humain dans certaines conditions (Chaochao Lu et Xiaou Tang 2014). De plus beaucoup de grands acteurs économiques s'attaquent au problème, obtiennent d'excellents résultats et en sortent des applications, par exemple Facebook avec deepFace en 2014, Microsoft avec Hello en 2015 et enfin très récemment Apple avec FaceID (en 2017).
Il à également été prouvé que la reconnaissance faciale pouvait être plus difficilement hackable que des mots de passe de par la complexité et l'unicité du visage humain.
\subsection{Notre solution : Face Key}
\subsubsection{Présentation}
Notre idée est de combiner les technologies émergentes avec un gestionnaire de mot de passe traditionnel afin de supprimer le mot de passe maître et de rendre instantanée et sécurisée la connexion à nos sites préférés.
\subsubsection{Fonctionnement}
Notre application se présentera sous forme d'un plug-in web, il suffira de se présenter sur la page du site où l'on veut se connecter et cliquer sur le plug-in en haut à droite du navigateur. L'application web va alors demander au programme faceKey qui tourne sur la machine de l'utilisateur de prendre une photo  ; un algorithme va trouver le visage présent d'en l'image et l'identifier. Si il s'agit bien du visage de l'utilisateur une requête sera faite aux bases de données des serveurs Face Key afin d'obtenir les identifiants pour la page visitée. La page s'actualise avec les identifiants du compte et d'un coup d'oeil vous êtes connecté.
...
Nous utiliserons une base de données pour stocker différentes informations et paramètres de nos utilisateurs. Par exemple les informations de connexion à la plateforme, les couples identifiant/password pour se connecter aux différents sites, différents paramètres de préférences de l'utilisateur, des données utilisateur collectées sur chaque site (heure moyenne de connexion, fréquence de connexion ...), des données sur l'utilisation de notre application(date de création, fréquence d'utilisation ...).
Un des objectifs est de rendre la base de données "anonyme", c'est-à-dire qu'on ne demande pas de nom, prénom à nos utilisateurs, uniquement une adresse mail, afin que si quelqu'un à accès aux données que nous collectons il ne puisse pas remonter à l'identité de nos utilisateurs.
...
La connexion entre le client et le serveur sera assurée par le protocole TCP permettant un transport des informations fiable et en mode connecté.
...
L'application Face Key est une application qui sera utile à la production du projet. Elle servira en effet à collecter les données pour l'apprentissage de nos modèles de reconnaissance faciale en recueillant des photos d'utilisateurs.
\subsubsection{Système d'exploitation}
Pour le projet de Systèmes d?exploitation nous avons fait le choix de supprimer les attentes actives au moment de la connexion via l?appareil photo.
\subsubsection{Projet de synthèse}
Pour la partie projet de synthèse, nous faisons le choix de ne pas juste apporter une interface au résultat des projets de réseau et BDD mais nous apportons allons chacun incorporer des domaines qui nous intéresse :
\begin{itemize}
	\item Louis L'Haridon : plug-in web/interface utilisateur
	\item Quentin Gerard : sécurité réseau/cryptographie
	\item Matthieu Vilain : machine learning/reconnaissance faciale
\end{itemize}
Ce choix est une prise de risque car il apporte une plus grosse charge de travail et nous force à apprendre par nous même des sujets que nous n'avons pas forcement vu en cours. Mais nous trouvons ça plus enrichissant et nous voulons en découvrir d'avantage dans des sujets qui nous intéressent pour notre avenir. 
...
En revanche si nous arrivons à tout finir en temps et en heure, nous nous laissons libre d'ajouter des fonctionnalités comme analyse de l'émotion de l'utilisateur lorsqu'il déverrouille l'application, l'ajout des statistiques ou autre.
L'idée initial était de pousser le projet à fond en développent un protocole d'installation, une documentation détaillé etc... afin de proposer le logiciel final en open source. Cet objectif ne pourra pas être réalisé au court du projet de L3 par manque de temps mais nous espérons pouvoir continuer à travailler dessus par la suite.

\newpage
\section{Gestion de projet}
\subsection{Répartition des tâches}

Comme dit plus haut, le projet entre dans le cadre du projet de synthèse de Licence 3, il englobe donc les matières de base de données, réseau, système d'exploitation et de développement Android. 
Nous avons décidé de travailler en commun sur les parties impliquent un cours que nous avons eu cette années. Nous avons donc tous travaillé sur la partie  réseau, base de données et sur l'application Android. Nous n'avons pas avancé dans notre projet de système d'exploitation, nous n'en parlerons donc pas dans ce rapport.
...
Cela nous permet d'ajouter à notre projet des domaines que nous n'avons pour l'instant pas vu en cours mais qui nous passionnent.

Le but est de laisser le plus de liberté possible à chacun dans sa partie, qu'il puisse expérimenter et tourner son problème de la manière qu'il veut. La seule contrainte étant que le travail puisse être intégrée dans un gestionnaire de mot de passe de quelconque façon.
...
Pour le code, nous nous sommes synchronisé sur GIT, et l'avancement du travail sur Trello.
...
\begin{figure}[H]
    \centering
        ...
        \caption{Localisation des blocs de BDD dans l'architecture de Face Key}
        \label{fig:ABDD}
\end{figure}
...
\hline
\end{tabular}
Les informations sont quant à elles séparées par une virgule, nous avons donc par exemple:
\begin{tabular}{|c|c|}
\hline
...
\newline
Ici les photos et les poids du réseau de neurones sont envoyés sous forme de fichier. Les fichiers sont ouverts par les deux programmes (client et serveur) et envoyés (et reçu) octet par octet sans code en entête. Le transfère d'un fichier est donc décomposé en plusieurs parties: tout d'abord les deux premiers messages envoie le nom du fichier ainsi que le nombre d'octet qui va être envoyé (la taille du fichier), ensuite les octet sont envoyés un par un. Le receveur compte le nombre d'octet reçu et s'arrête une fois que l'intégralité des octet ont bien été reçu.
\begin{figure}[h]
	\centering
...
\textit{Pour démarrer ce diagramme il faut préalablement envoyer le code diagramme (code 001 pour la connexion à un site) au serveur \textbf{et} effectuer une phase d'authentification (code 103).}
\begin{figure}[h]
	\centering
...
\textit{Pour démarrer ce diagramme il faut préalablement envoyer le code diagramme (code 003) au serveur \textbf{et} effectuer une phase d'authentification (code 103).}
\newline
...
\begin{figure}[H]
    \centering
        \includegraphics[width=0.8\textwidth]{img/archi_os.png} 
        \caption{Localisation des blocs de systèmes d'exploitation}
        \label{fig:schefac}
\end{figure}

Dans l?objectif de supprimer les attentes actives au moment de la connexion nous allons travailler sur notre client C.
Dans un premier on temps on va forker un processus qui s?occupera de charger le modèle de reconnaissance. Ce processus attendra ensuite l?instruction connect ou l?instruction quit tu processus principal. Si il reçoit quit, il se ferme et le client se ferme. Si il reçoit connect, le processus forcé va prendre une photo, verifier la photo par rapport au modèle et renvoyer au processus principal de taux d?appartenance au modèle ainsi que les taux d?appartenance à tous les autres modèles. Il va ensuite reboucler et attendre l?instruction connect oui l?instruction quit. 
\begin{figure}[H]
    \centering
        \includegraphics[width=0.8\textwidth]{img/OS.PNG} 
\end{figure}
...
\end{lstlisting}
Et, en cliqaunt sur le bouton soumettre on vérifie si le couple login/mdp correspond à un autre couple présent dans le tableau.  
\begin{lstlisting}[language=java]
	for(String[]s : login) {
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.4\textwidth}
    ...
    \caption{Envoi de photos au serveur}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{img/blob.png}
    \caption{Réception des photos sur la BDD}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Localisation des blocs du plug-in dans l'architecture de Face Key}
        \label{fig:AC}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Schématisation du fonctionnement d'une application Chrome}
        \label{fig:APPC}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Schématisation du fonctionnement d'une extension Chrome}
        \label{fig:EXT}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Capture d'écran du plug-in utilisé sur une page de connexion Facebook}
        \label{fig:extens}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Architecture de notre plug-in}
        \label{fig:EXT}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Architecture possible de Face Key avec une extension \`a la place du client C}
        \label{fig:newarc}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Calculateur graphique appelé "Brutus" pour sa puissance de calcul} 
        \label{fig:brutus}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Système de localisation du visage utilisant Haar Cascade}
        \label{fig:mat}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.3\textwidth}
    ...
    \caption{Features simples}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{img/ms2.png}
    \caption{Description d'un visage à partir des features simples}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
    \centering
        \includegraphics[width=0.85\textwidth]{img/ms3.png} 
        \caption{Exemple de transformations utilisées pour l'augmentation de données}
        \label{fig:ms3}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Exemple de visages reconnu et d'erreurs faites par Harr Cascade}
        \label{fig:ms3}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Neurone et perceptron simple} 
        \label{fig:ms5}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Affichage des poids de chaque classe d'une régression logistique}
        \label{fig:ms6}
\end{figure}
...
C'est ce qu'apporte l'arrivée des réseaux de neurones convolutif (CNN, Y.Lecun 1998) et le développement du deeplearning. Le principe des CNN est d'ajouter des couches dites de convolutions avant un perceptron multicouches.

Le principe de la convolution est d'utiliser des noyaux que l'on va convoluer sur l'image d'entrée pour obtenir des filtres. Dans un filtre obtenu, chaque valeur sera le taux de correspondance entre le noyau et la partie de l'image centré en ce point.


\begin{figure}[H]
    \centering
        \includegraphics[width=0.6\textwidth]{img/ms8.png} 
        \caption {Exemple de noyaux}
        \label{fig:ms8}
\end{figure}
...
\begin{itemize}
	\item Le padding : Si l'on effectue une convolution simple sur l'image (avec un 'same padding') on aura une réduction de dimension des filtres obtenu par rapport à l'image d'entrée. On peut donc entourer notre image d'entrée de 0 pour obtenir la même dimension en sortie ('zero padding', utilisé dans l'exemple ci-dessus)
	\item Le stride : Le décalage en pixel du noyau lorsqu'on le 'glisse' pendant l'opération de convolution. Le stride a un impact sur la taille des filtres de sortie.
\end{itemize}

On peut par la suite enchainer les couches de convolution en utilisant les filtres comme image d'entrée pour la couche suivante. Les couches de convolution vont permettre de décrire l'image en différents pattern. On va donc avoir des filtres qui permettent de décrire de plus en plus précisément notre image originale. 
...
\begin{figure}[H]
    \centering
        ...
        \caption{Normalisation par fonction ReLU}
        \label{fig:ms13}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Différents types de descente du gradient}
        \label{fig:ms14}
\end{figure}

\paragraph{Implémentation}

Cette fois ci nous n'avons pas implémenté ce modèle from scratch mais nous avons utilisé la libraire Keras qui tourne avec Tensorflow pour un entrainement rapide sur carte graphique.

Nous voulions initialement implémenter les mêmes architectures que DeepFace et FaceNet mais au moment de la création de la base de données nous avons sauvegarder les images en 100x100 alors que Deepface utilise des images de 152x152 et FaceNet 220x220.

Nous avons donc décidé de concevoir notre propre architecture. Nous décidons également de rester sur des images en noir et blanc, en pensant que l'humain ne nécessite pas la couleur pour identifier des gens sur une photo. Nous voulions que notre réseau 'se focalise' sur les formes des visages. Nous sommes conscients que c'est un partie-prit osé de par la quantité d'informations que l'on retire. Les résultats des expériences présentées si dessous sont donc réalisées sur des images noir et blanc, de nouveaux résultats seront peut être présentés en couleur pour la présentation.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|l|} 
		\hline
		\textbf{Couches} & \textbf{Description} \\
		\hline
		...
		SoftMax & {} \\
		\hline
		\multicolumn{2}{|l|}{\textbf{Total du poids} : 2 052 000} \tabularnewline
		\hline
	\end{tabular}	 
	\label{Table:CTEW}
\end{table}
...
\paragraph{Résultats}
Avec le modèle décrit ci-dessus on obtient 97,1\% de reconnaissance sur notre base de données. Avec ce niveau de reconnaissance on se rapproche des résultats de DeepFace(97,3\%) et FaceNet(99,6\%), ce qui est logique car on utilise le même type de réseau qu'eux.
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.4\textwidth}
    ...
    \caption{Évolution de l'accuracy pendant l'entrainement}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{img/ms16.png}
    \caption{Évolution du loss pendant l'entrainement}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
    \centering
        ...
        \caption{Matrice de confusion, valeurs en nombre d'images}
        \label{fig:ms17}
\end{figure}

On remarque qu'il y à très peu de confusion entre les classes. De plus certaines confusion sont explicable, par exemple il y a eu 155+12=167 erreur entre la classe c et la classe k :

\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.2\textwidth}
    \includegraphics[width=\textwidth]{img/ms18.png}
    \caption{Individu de la classe k}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.2\textwidth}
    \includegraphics[width=\textwidth]{img/ms19.png}
    \caption{Individu de la classe c}
  \end{minipage}
\end{figure}

Les deux individus ont une forme de visage assez ronde, des yeux qui se ressemblent, des cheveux longs ... Les erreurs commise entre ces deux classes sont donc compréhensible (ce nombre d'erreur reste très faible par rapport au nombre d'images total, <1%)
...
\begin{figure}[H]
    \centering
        ...
        \caption{Essai sur une photo n'appartenant pas à la base de données} 
        \label{fig:ms20}
\end{figure}
...
Nous n'avons malheureusement pas pu faire de réels tests chiffrés pour l'utilisation du modèle dans le monde réel. Nous n'avons donc aucun moyen de savoir de manière fiable la performance de notre solution de reconnaissance faciale. Nous espérons pouvoir faire ces tests avant la soutenance.

\subsection{Conclusion : remarques, avancement et améliorations possibles}

Nous remarquons que la plupart de nos problèmes viennent de la mal formation de la base de données. Nous avons passé beaucoup de temps pour la constituer mais nous avons sans doute manqué un temps de recherche sur comment bien constituer une base de données. 

La finition de cette partie n'est pas parfaite, nous avons passé beaucoup de temps à nous documenter sur les modèles, constituer la base, choisir les modèles, prendre en mains les outils et implémenter tous les algorithmes (y compris ceux qui ne se voient pas sur la manipulation de données pour la construction de la base). Il ne nous restait donc plus beaucoup de temps pour incorporer proprement la solution dans l'application et chiffrer nos résultats.

Au niveau des améliorations possible nous pensons bien évidement à la refonte totale de la base de données (nombre d'images, taille, augmentation différentes). Nous pensons également à la recherche d'un modèle réellement efficace, voir si FaceNet marche aussi bien sur une base de données maison (99,63\% ont été obtenu sur la base de données Labeled Faces in the Wild LFW). Et enfin une nouvelle méthode de localisation des visages, implémentation de HOG pour tester ou utiliser un réseau qui localise également le visage (Object detector, exemple réseau YOLO). Il serait également intéressante d'analyser l'expression faciale de l'utilisateur.
...
La fonction de décryptage est tout aussi simple, à partir de notre message crypter $C$ , et de notre clé privée \((d, n)\), nous calculons: \[M \equiv C^d (mod \phantom{s}n)\]
...
Les clés publique et privées du serveur et du client sont généré préalablement, et stocké dans les fichiers des deux programmes.
\newline{}
\paragraph{Échange des clés}
Lors de chaque connexion du client au serveur, les clés publiques des deux parties sont échangées, envoyé sous forme de fichier "\textit{.pem}" et stocké, avec les autres clés pour le client, et dans un dossier nommé par le PID du processus pour le serveur (le serveur étant multi-processus).
\newline{}
...
Au sein du projet Face Key, nous utiliserons une fonction de hachage (MD5) pour transformer les valeurs stockées dans la base donnée et régulièrement envoyé par l'utilisateur tel que: son mot de passe. Ainsi à chaque fois que l'utilisateur enverra son mot de passe, nous récupérerons la valeur hachée stockée dans la base, et nous transformerons le mot de passe envoyé et vérifierons les 2 sommes de vérifications. Si les 2 sommes sont les mêmes alors le mot de passe donnée est le même sinon le mot de passe est erroné.
Cependant nous pouvons remarquer qu'une fonction de hachage n'est pas suffisante afin de garantir une sécurité satisfaisante. En effet, si la base venait à être compromise et lu par une personne tierce, il lui suffirait d'utiliser une liste des mot de passe les plus utiliser préalablement hachés, et de seulement comparer les sommes de vérifications à celle dans la base donnée pour révéler plusieurs mot de passe. Sachant que 2 même mot de passe obtiennent la même sortie cela diminue grandement le temps nécéssaire à retrouver plusieurs mot de passes. De plus, il existe certaines méthodes pour retrouver des mot de passes hachés en clair, comme les attaques par dictionnaires ou Rainbow Table. Nous devons donc trouver une solution pour rendre la tache plus compliqué.
\begin{figure}[h]
	\centering
...
Le principe du haching salé est simple: ajouter une chaine de caractère que l'on va concaténer au mot de passe avant de hacher la chaine final. Chaque utilisateur a un sel unique stocker sur la base de donnée. Ainsi 2 même mot de passes n'auront pas la même chaine hachée. Cela rend les attaques par Brute Force, Dictionnaire et Rainbow Table, bien plus fastidieuse en augmentant le temps nécéssaire pour retrouver les mot de passes.
\begin{figure}[h]
	\centering
...
Pour commencer ce bilan, faisons un point sur ce qui est implémenté ou non. Nous avons vu que le RSA est partiellement implémenté, chaque bloc fonctionne de manière indépendante, mais la transmission du message crypté ne s'effectue pas bien. Pour ce qui est du hachage, seul le hachage classique est implémenté, le hachage salé n'est pas implémenté par manque de temps. En effet trouver d'où venait le problème du RSA nous à pris pas mal de temps, et nous ne pouvons à l'heure actuelle, estimer combien de temps il faudrais pour le résoudre, car nous ne connaissons pas la cause du problème. L'implémentation du Hachage Salé nécessiterait une modification de la base de donnée, pour stocker le "sel" de chaque utilisateur.
De plus, il resterait d'autres problèmes de sécurité à régler, même si nous avions pu tout implémenté. En effet les combinaisons Identifiants/Mot de passes pour chaque site web d'un utilisateur ne sont pas protégés ! Il aurait aussi été interessant de crypter l'envoie et la sauvegarde des fichiers (images et réseau de neurones) sur le serveur, afin de préserver l'identité des utilisateurs.


\newpage
\section{Conclusion}

Face Key est le premier gros projet informatique que nous avons réalisé. La façon dont on voyait ce projet depuis le début était ambitieuse car nous avons décidé de ne pas simplement faire une interface à notre projet de réseau/BDD mais nous sommes partit sur 3 axes différents : L'interface utilisateur, la sécurité et le machine learning. Nous avons clairement sous-estimé la charge de travail que ça allez nous apporter et sur-estimé le temps que nous avions pour la réalisation. Cependant nous avons réussi à sortir un prototype relativement complet de notre application alors que nous nous étions lancé dans des domaines que nous connaissions peu et dont nous n'avons pas encore suivi de cours.
\\\\
Nous sortons comptent de ce projet car pour un projet de synthèse, il à réellement regroupé une grande majorité des cours que nous avons suivis pendant nos 3 années de licence : Réseau, base de données, développement Android, algorithmique, génie logiciel, web, algèbre linéaire, traitement d'images, traitement du signal, gestion de projet informatique, recherche bibliographique et maths pour l'informatique. De plus il nous à aussi permis de nous projeter dans les domaines dans lesquels nous voulions travailler plus tard. En effet en début de projet nous avions choisi les modules que nous allions travailler en fonction de ce qui nous intéressé le plus en informatique. Ce projet à permis à certains d'entre nous de renforcer leurs choix mais a aussi mis de doute à d'autres.
\\\\
L'application n'est pas fini,  certains détails reste à implémenter et des améliorations sont possible dans chacune de nos parties. Il y à également des questions qui reste sans réponses pour le moment comme, que se passe-t-il si on augmente notre nombre d'utilisateurs ? L'architecture du réseau sera-t-elle suffisante ? cela va-t-il créer de nouvelle faille de sécurité ? Le modèle de reconnaissance faciale fonctionnera-t-il toujours ? Y a-t-il des solutions technique et théorique plus performante à nos problématiques ?
